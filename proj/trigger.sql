...
-- single olympic. But, assume it is there.
...
FROM V_SCOREBOARD_WITH_COUNTRY
...
CREATE OR REPLACE VIEW V_COUNTRY_MEDAL_COUNT_BY_TYPE AS
...
FROM V_COUNTRY_MEDAL_COUNT VCMC LEFT JOIN MEDAL M on VCMC.MEDAL_ID = M.MEDAL_ID
...
LEFT JOIN PARTICIPANT P ON S.PARTICIPANT_ID = P.PARTICIPANT_ID;


CREATE OR REPLACE VIEW V_PARTICIPANT_MEDAL_COUNT AS
SELECT OLYMPIC_ID, PARTICIPANT_ID, NAME, MEDAL_ID, MEDAL_TITLE, MAX(MEDAL_COUNT) AS MEDAL_COUNT FROM
((SELECT OLYMPIC_ID, PARTICIPANT_ID, NAME, MEDAL_ID, MEDAL_TITLE, COUNT(*) AS MEDAL_COUNT
FROM V_SCOREBOARD_WITH_PARTICIPANT
NATURAL JOIN MEDAL MEDAL_ID
...
SELECT DISTINCT C.OLYMPIC_ID, C.NAME, C.PARTICIPANT_ID,
(SELECT MEDAL_COUNT FROM V_PARTICIPANT_MEDAL_COUNT MC
    WHERE MEDAL_TITLE='gold'
    AND MC.OLYMPIC_ID=C.OLYMPIC_ID
    AND MC.PARTICIPANT_ID=C.PARTICIPANT_ID) AS GOLD_MEDAL_COUNT,
(SELECT MEDAL_COUNT FROM V_PARTICIPANT_MEDAL_COUNT MC
    WHERE MEDAL_TITLE='silver'
    AND MC.OLYMPIC_ID=C.OLYMPIC_ID
    AND MC.PARTICIPANT_ID=C.PARTICIPANT_ID) AS SILVER_MEDAL_COUNT,
(SELECT MEDAL_COUNT FROM V_PARTICIPANT_MEDAL_COUNT MC
    WHERE MEDAL_TITLE='bronze'
    AND MC.OLYMPIC_ID=C.OLYMPIC_ID
    AND MC.PARTICIPANT_ID=C.PARTICIPANT_ID)
       AS BRONZE_MEDAL_COUNT
FROM V_PARTICIPANT_MEDAL_COUNT C GROUP BY OLYMPIC_ID, PARTICIPANT_ID, NAME, MEDAL_TITLE, MEDAL_COUNT;

CREATE OR REPLACE VIEW V_PARTICIPANT_TOTAL_POINTS AS
SELECT OLYMPIC_ID, PARTICIPANT_ID, NAME, SUM(MEDAL_COUNT * POINTS) AS TOTAL_POINTS
FROM V_PARTICIPANT_MEDAL_COUNT VPMC LEFT JOIN MEDAL M on VPMC.MEDAL_ID = M.MEDAL_ID
GROUP BY OLYMPIC_ID, PARTICIPANT_ID, NAME ORDER BY OLYMPIC_ID, TOTAL_POINTS DESC, NAME;

CREATE OR REPLACE VIEW V_OLYMPIC_PARTICIPANT AS
SELECT DISTINCT O.OLYMPIC_ID, P.PARTICIPANT_ID from TEAM T JOIN TEAM_MEMBER TM on
    T.TEAM_ID = TM.TEAM_ID
    JOIN PARTICIPANT P on TM.PARTICIPANT_ID = P.PARTICIPANT_ID
    JOIN OLYMPICS O on T.OLYMPIC_ID = O.OLYMPIC_ID;

-- Assumption: in case of ties, the topKAthletes() function separates
-- ties based on their name in alphabetical order.
CREATE OR REPLACE VIEW V_PARTICIPANTS_RANKING AS
SELECT R.OLYMPIC_ID, -- This view includes participants for years they were not competing
       R.PARTICIPANT_ID,
        RANK() OVER (
        PARTITION BY R.OLYMPIC_ID
        ORDER BY TOTAL_POINTS DESC
        ) RANK,
       NAME,
       TOTAL_POINTS AS POINTS,
       ...
       SILVER_MEDAL_COUNT AS SILVER,
       BRONZE_MEDAL_COUNT AS BRONZE
FROM (SELECT * FROM (V_PARTICIPANT_MEDAL_BY_TYPE NATURAL JOIN
    V_PARTICIPANT_TOTAL_POINTS PARTICIPANT_ID)) R
